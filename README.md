...
`/discover/` 是站内五方向论文发现入口，覆盖具身智能、人工智能、计算与语言、计算机视觉和机器学习。直接访问默认具身智能，也可用 `?field=cs-ai` 等参数打开指定方向。正式精读库仍只来自 `D:\essay`；发现页候选箱使用浏览器 `localStorage`，不会直接写回论文库。
...
脚本以 arXiv 为必须来源，并用 Semantic Scholar 和 OpenReview 补充元数据。`SEMANTIC_SCHOLAR_API_KEY` 是可选环境变量；未配置时会使用未认证接口并严格限速。每个方向只接收至少匹配一个核心主题的论文，独立保留最近 730 天、最多 2,000 篇；发布超过 180 天的论文只有在方向与主题相关度至少 24、研究证据至少 14、信息完整度至少 12 时才进入快照。Semantic Scholar 元数据全局批量获取；某方向至少有 5 篇已读且带 arXiv ID 的正式库论文后才启用相似推荐。

schema v3 将基础阅读分与个性化匹配分开：方向与主题 30、研究证据 45、信息完整度 20、新鲜度 5；Semantic Scholar 推荐和浏览器本地 `±8` 反馈只在同一阅读等级内重排。发现页在无关键词时按近 180 天与更早论文 `1:2` 交错展示，并支持方向切换、起始日期、截止日期及快捷时间范围组合筛选；输入关键词后优先按文本相关度排序。顶部只展示基础分至少 70 且研究证据达标的候选，不会跨年龄段填满缺失位置。

只需对现有候选执行确定性重评分而不访问外部 API 时，可设置 `DISCOVERY_RESCORE_ONLY=1` 后运行同一刷新命令；该模式用于 schema/评分版本迁移，不替代日常联网刷新。
...
- `src/data/generated/discovery.json`：具身智能静态快照及旧接口兼容数据。
- `src/data/generated/discovery-cs-*.json`、`discovery-index.json`：其他四个方向的按需加载快照与轻量索引。
